...
:: ^T
if %KEY% == 20 call taskmon.bat _GETANSWER "Transparency [1-255]:" & (if "!ANSWER!"=="" exit /b 0) & set /a NUMA="!ANSWER!" & (if !NUMA! geq 1 if !NUMA! leq 255 call :GET_TARGET !FO%CURRPOS%! 2 & nircmdc.exe win activate process /!TARGET! & nircmdc.exe win trans process /!TARGET! !NUMA! ) & exit /b 0
...
if %LKEY% == "C" call taskmon.bat _GETANSWER "Window title:" & (if "!ANSWER!"=="" exit /b 0) & call :GET_TARGET !FO%CURRPOS%! 2 & nircmdc.exe win activate process /!TARGET! & nircmdc.exe win settext process /!TARGET! "!ANSWER!" & exit /b 0
...
if %LKEY% == "P" call taskmon.bat _GETANSWER "Priority [normal, low, belownormal, abovenormal, high, realtime]:" & (if "!ANSWER!"=="" exit /b 0) & call :GET_TARGET !FO%CURRPOS%! 2 & nircmdc.exe setprocesspriority /!TARGET! !ANSWER! & exit /b 0
